...
func runServer(args []string) {
	...
	httpClient := &http.Client{
		...
		Transport: &http.Transport{
			DialContext:         netutil.SSRFSafeDialer(),
			MaxIdleConns:        100,
...
func setupRoutes(g *fastglue.Fastglue, app *handlers.App, lo logf.Logger, basePath string, rdb *redis.Client, cfg *config.Config) {
	...
	g.POST("/api/org/logo", app.UploadOrgLogo)
	g.DELETE("/api/org/logo", app.DeleteOrgLogo)
	...
	// Super-admin cross-org management (each handler enforces IsSuperAdmin itself)
	g.POST("/api/organizations/with-admin", app.CreateOrganizationWithAdmin)
	g.PUT("/api/organizations/{id}", app.UpdateOrganization)
	g.DELETE("/api/organizations/{id}", app.DeleteOrganization)
	g.POST("/api/organizations/{id}/suspend", app.SuspendOrganization)
	g.POST("/api/organizations/{id}/activate", app.ActivateOrganization)
	...
	g.GET("/api/data-connections", app.ListDataConnections)
	g.POST("/api/data-connections", app.CreateDataConnection)
	...
	g.POST("/api/knowledge-bases", app.CreateKnowledgeBase)
	g.PUT("/api/knowledge-bases/{id}", app.UpdateKnowledgeBase)
	g.DELETE("/api/knowledge-bases/{id}", app.DeleteKnowledgeBase)
	g.GET("/api/knowledge-bases/{id}/documents", app.ListDocuments)
	g.POST("/api/knowledge-bases/{id}/documents", app.CreateDocument)
	g.GET("/api/knowledge-bases/{id}/documents/{doc_id}", app.GetDocument)
	g.DELETE("/api/knowledge-bases/{id}/documents/{doc_id}", app.DeleteDocument)
	g.POST("/api/knowledge-bases/{id}/documents/{doc_id}/retry", app.RetryDocument)
