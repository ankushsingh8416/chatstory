...
// requireSuperAdmin extracts the user ID and verifies super admin status.
// Mirrors requireAuth, but for cross-org endpoints (e.g. super-admin
// organization management) where org-scoped permission checks don't apply —
// only a super admin may proceed regardless of which org they're in.
func (a *App) requireSuperAdmin(r *fastglue.Request) (userID uuid.UUID, err error) {
