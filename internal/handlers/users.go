...
// ResetUserPassword generates a new random password for another user and
// returns it once in the response. This is the audited, admin-facing
// equivalent of UpdateUser's implicit password field — kept as a distinct
// action so password resets show up clearly in the audit log and so the
// plaintext is never something an admin has to invent or transmit themselves.
// The password is never stored or logged anywhere; only its bcrypt hash is
// persisted, matching every other password path in this file.
func (a *App) ResetUserPassword(r *fastglue.Request) error {
	...
	if targetID == currentUserID {
		return r.SendErrorEnvelope(fasthttp.StatusBadRequest, "Use the change-password form to reset your own password", nil, "")
	}

	isSuperAdmin := a.IsSuperAdmin(currentUserID)

	var user models.User
	var auditOrgID uuid.UUID

	if isSuperAdmin {
		// Super admins can reset any user's password, in any organization.
		if err := a.DB.Where("id = ? AND deleted_at IS NULL", targetID).First(&user).Error; err != nil {
			return r.SendErrorEnvelope(fasthttp.StatusNotFound, "User not found", nil, "")
		}
		...
		if !a.HasPermission(currentUserID, models.ResourceUsers, models.ActionReset, orgID) {
			return r.SendErrorEnvelope(fasthttp.StatusForbidden, "Insufficient permissions", nil, "")
		}
		// Scope to the caller's own org, same as UpdateUser/DeleteUser.
		if err := a.DB.
			Select("users.*").
			Joins("JOIN user_organizations ON user_organizations.user_id = users.id AND user_organizations.organization_id = ? AND user_organizations.deleted_at IS NULL", orgID).
			Where("users.id = ? AND users.deleted_at IS NULL", targetID).
			First(&user).Error; err != nil {
			return r.SendErrorEnvelope(fasthttp.StatusNotFound, "User not found", nil, "")
		}
		auditOrgID = orgID
	...
	if err := a.DB.Model(&models.User{}).Where("id = ?", user.ID).Update("password_hash", string(hashedPassword)).Error; err != nil {
		a.Log.Error("Failed to reset password", "error", err)
		return r.SendErrorEnvelope(fasthttp.StatusInternalServerError, "Failed to reset password", nil, "")
	}

	a.logAudit(auditOrgID, currentUserID, "user", user.ID, models.AuditActionUpdated,
		map[string]any{"action": "password_reset"},
		map[string]any{"action": "password_reset", "target_email": user.Email})

	return r.SendEnvelope(map[string]any{
		"message":  "Password reset successfully. Share it with the user securely — it will not be shown again.",
