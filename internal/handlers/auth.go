...
func (a *App) Login(r *fastglue.Request) error {
	...
	// Block sign-in for a suspended organization. Super admins are exempt so
	// they can still sign in to manage/reactivate it.
	if !user.IsSuperAdmin {
		var org models.Organization
		if err := a.DB.Where("id = ?", user.OrganizationID).First(&org).Error; err == nil {
			if suspended, _ := org.Settings["suspended"].(bool); suspended {
				return r.SendErrorEnvelope(fasthttp.StatusForbidden, "This organization has been suspended. Contact support.", nil, "")
			}
		}
	}
