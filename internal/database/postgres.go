...
func GetMigrationModels() []MigrationModel {
	return []MigrationModel{
		...
		// Data Connections (bring-your-own DB/vector store)
		{"DataConnection", &models.DataConnection{}},

		// Knowledge Base (RAG documents for chatbot retrieval)
...
func RunMigrationWithProgress(db *gorm.DB, adminCfg *config.DefaultAdminConfig) error {
	...
	// Backfill permissions added after an org's roles were first seeded, so
	// upgrades reach pre-existing orgs too, not just newly created ones.
	if err := BackfillRolePermissions(silentDB, models.ResourceUsers, models.ActionReset, []string{"admin"}); err != nil {
		fmt.Printf("\n  \033[31m✗ Failed to backfill users:reset permission\033[0m\n\n")
		return err
	}
	for _, action := range []string{models.ActionRead, models.ActionWrite, models.ActionDelete, models.ActionExecute} {
		if err := BackfillRolePermissions(silentDB, models.ResourceDataConnections, action, []string{"admin", "manager"}); err != nil {
			fmt.Printf("\n  \033[31m✗ Failed to backfill data_connections:%s permission\033[0m\n\n", action)
			return err
		}
	}
	for _, action := range []string{models.ActionRead, models.ActionWrite, models.ActionDelete} {
		if err := BackfillRolePermissions(silentDB, models.ResourceKnowledgeBase, action, []string{"admin", "manager"}); err != nil {
			fmt.Printf("\n  \033[31m✗ Failed to backfill knowledge_base:%s permission\033[0m\n\n", action)
			return err
		}
	}
...
// BackfillRolePermissions grants a specific permission to existing system
// roles that don't already have it, without touching any of their other
// permissions. FixSystemRolePermissions only links permissions for roles
// that currently have zero permissions at all (fresh installs), so a
// permission added after an org already exists would otherwise never reach
// that org's roles — this closes that gap for upgrades.
func BackfillRolePermissions(db *gorm.DB, resource, action string, roleNames []string) error {
	var perm models.Permission
	if err := db.Where("resource = ? AND action = ?", resource, action).First(&perm).Error; err != nil {
		return fmt.Errorf("failed to find permission %s:%s: %w", resource, action, err)
	}

	var roles []models.CustomRole
	if err := db.Where("is_system = ? AND name IN ?", true, roleNames).Find(&roles).Error; err != nil {
		return fmt.Errorf("failed to fetch roles: %w", err)
	}

	for _, role := range roles {
		var count int64
		db.Table("role_permissions").Where("custom_role_id = ? AND permission_id = ?", role.ID, perm.ID).Count(&count)
		if count > 0 {
			continue // Already has it
		}
		if err := db.Model(&role).Association("Permissions").Append(&perm); err != nil {
			return fmt.Errorf("failed to backfill %s:%s for role %s: %w", resource, action, role.Name, err)
		}
	}
