...
api.interceptors.request.use(
  (config: InternalAxiosRequestConfig) => {
    ...
    // Add organization override header for org switching. A caller that
    // already set this header explicitly (e.g. a super admin managing a
    // specific org from the admin dashboard, without switching into it) wins
    // over the globally selected org — don't clobber it.
    const selectedOrgId = localStorage.getItem('selected_organization_id')
...
export const usersService = {
  ...
  resetPassword: (id: string) => api.post<{ message: string; password: string }>(`/users/${id}/reset-password`),
}
...
export const organizationService = {
  ...
  updateSettings: (data: {
    ...
    // Used for Knowledge Base document embeddings only — independent of
    // whichever provider (OpenAI/Anthropic) the chatbot's replies use.
  ...
  // orgId lets a super admin update a different org's logo (via the
  // X-Organization-ID override) without switching into it — omit it to
  // act on the caller's own current org.
  uploadLogo: (file: File, orgId?: string) => {
    const formData = new FormData()
    formData.append('file', file)
    return api.post<{ message: string; logo_data_url: string }>('/org/logo', formData, {
      headers: {
  ...
  deleteLogo: (orgId?: string) => api.delete('/org/logo', orgId ? orgHeader(orgId) : undefined)
}
...
// orgHeader builds a per-call config that scopes the request to a specific
// organization via X-Organization-ID, overriding the globally selected org.
// Only super admins can target an org they aren't a member of — the backend
// enforces that; this is purely a request-shaping helper.
function orgHeader(orgId: string) {
  return { headers: { 'X-Organization-ID': orgId } }
}
...
export const organizationsService = {
  ...
  createWithAdmin: (data: { org_name: string; admin_email: string; admin_full_name: string; admin_password?: string }) =>
    api.post<Organization & { admin_email: string; generated_password?: string }>('/organizations/with-admin', data),
  update: (id: string, data: { name?: string }) => api.put<Organization>(`/organizations/${id}`, data),
  delete: (id: string) => api.delete(`/organizations/${id}`),
  suspend: (id: string) => api.post<{ message: string; suspended: boolean }>(`/organizations/${id}/suspend`),
  activate: (id: string) => api.post<{ message: string; suspended: boolean }>(`/organizations/${id}/activate`),
  // Members — pass orgId to manage a specific org's members without switching into it
  listMembers: (orgId?: string, params?: { search?: string; page?: number; limit?: number }) =>
    api.get<{ members: OrganizationMember[]; total: number }>('/organizations/members', {
      params,
      ...(orgId ? orgHeader(orgId) : {})
    }),
  addMember: (data: { user_id?: string; email?: string; role_id?: string }, orgId?: string) =>
    api.post('/organizations/members', data, orgId ? orgHeader(orgId) : undefined),
  removeMember: (memberId: string, orgId?: string) =>
    api.delete(`/organizations/members/${memberId}`, orgId ? orgHeader(orgId) : undefined),
}
...
// Data Connections (bring-your-own DB/vector store for the chatbot knowledge base)
export type DataConnectionType = 'postgres' | 'qdrant'
export type DataConnectionStatus = 'untested' | 'ok' | 'failed'
...
// password/qdrant_api_key: omit or send empty to leave the stored credential
// unchanged on update; required (for the relevant type) on create.
...
export type KBDocumentSourceType = 'upload' | 'text'
...
export const knowledgeBasesService = {
  list: () => api.get<{ knowledge_bases: KnowledgeBase[] }>('/knowledge-bases'),
  create: (data: KnowledgeBaseRequest) => api.post<KnowledgeBase>('/knowledge-bases', data),
  update: (id: string, data: KnowledgeBaseRequest) => api.put<KnowledgeBase>(`/knowledge-bases/${id}`, data),
  delete: (id: string) => api.delete(`/knowledge-bases/${id}`),
  listDocuments: (kbId: string) => api.get<{ documents: KBDocument[] }>(`/knowledge-bases/${kbId}/documents`),
  getDocument: (kbId: string, docId: string) => api.get<KBDocument>(`/knowledge-bases/${kbId}/documents/${docId}`),
  createTextDocument: (kbId: string, data: { name: string; text: string }) =>
    api.post<KBDocument>(`/knowledge-bases/${kbId}/documents`, data),
  uploadDocument: (kbId: string, file: File, name?: string) => {
    const formData = new FormData()
    formData.append('file', file)
    if (name) formData.append('name', name)
    return api.post<KBDocument>(`/knowledge-bases/${kbId}/documents`, formData, {
      headers: { 'Content-Type': 'multipart/form-data' }
    })
  },
  retryDocument: (kbId: string, docId: string) => api.post(`/knowledge-bases/${kbId}/documents/${docId}/retry`),
