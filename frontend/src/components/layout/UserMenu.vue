    ...
    <Popover v-model:open="isUserMenuOpen">
      <PopoverTrigger as-child>
        <Button
          variant="ghost"
          :class="[
            'flex items-center justify-start w-full h-auto px-2 py-1.5 gap-2 hover:bg-white/[0.08]',
            collapsed && 'md:justify-center'
          ]"
          aria-label="User menu"
        >
          <Avatar class="h-9 w-9 ring-2 ring-white/10">
          ...
          <div v-if="!collapsed" class="flex flex-col items-start text-left">
            <span class="text-[13px] font-medium truncate max-w-[140px] text-white">
              {{ authStore.user?.full_name }}
            </span>
            <span class="text-[11px] text-white/75 truncate max-w-[140px]">
