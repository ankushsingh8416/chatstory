...
const canReset = computed(() => authStore.hasPermission('users', 'reset'))
...
// Reset password (one-time password reveal, mirrors the admin org-management flow)
...
async function resetPassword() {
  ...
  try {
    const response = await usersService.resetPassword(user.value.id)
      ...
      <template #actions>
        <div class="flex items-center gap-2">
          ...
          >
            ...
            {{ isResettingPassword ? $t('common.resettingPassword') : $t('common.resetPassword') }}
          </Button>
          <Button
        ...
        <div class="space-y-4 py-4">
          <div class="space-y-2">
            ...
            <div class="flex gap-2">
              <Input :model-value="resetPasswordResult" readonly class="font-mono text-sm" />
              <IconButton :icon="Copy" :label="$t('common.copy')" variant="outline" @click="copyToClipboard(resetPasswordResult)" />
            </div>
          </div>
        </div>
        <DialogFooter>
          <Button size="sm" @click="resetPasswordDialogOpen = false">{{ $t('common.done') }}</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  </div>
