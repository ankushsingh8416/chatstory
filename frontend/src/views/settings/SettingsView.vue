...
import { Settings, Bell, Loader2, Globe, Phone, Upload, Play, Pause, Music, Building2 } from 'lucide-vue-next'
...
const canWriteKnowledgeBase = computed(() => authStore.hasPermission('knowledge_base', 'write'))
...
const generalSettings = ref({
  ...
  has_openai_embeddings_key: false,
  logo_data_url: ''
})
  ...
  try {
    ...
    if (orgData) {
      generalSettings.value = {
        ...
        openai_embeddings_key: '',
        has_openai_embeddings_key: orgData.settings?.has_openai_embeddings_key || false,
        logo_data_url: orgData.settings?.logo_data_url || ''
      }
...
async function saveGeneralSettings() {
  ...
  try {
    ...
    if (canWriteKnowledgeBase.value && generalSettings.value.openai_embeddings_key) {
      payload.openai_embeddings_key = generalSettings.value.openai_embeddings_key
    }
    ...
    generalSettings.value.openai_embeddings_key = ''
    // Refresh organization settings to update has_*_secret status
    ...
    if (orgData) {
      ...
      generalSettings.value.has_openai_embeddings_key = orgData.settings?.has_openai_embeddings_key || false
    }
...
async function uploadLogo(event: Event) {
  ...
  try {
    ...
    generalSettings.value.logo_data_url = data.logo_data_url
...
async function removeLogo() {
  ...
  try {
    ...
    generalSettings.value.logo_data_url = ''
      ...
      <div class="p-6 space-y-4 max-w-4xl mx-auto">
        <Tabs default-value="general" class="w-full">
          ...
          <TabsContent value="general">
              ...
              <div class="p-6 pt-3 space-y-4">
                ...
                <div class="space-y-2">
                  ...
                  <div class="flex items-center gap-4">
                    <div class="h-16 w-16 rounded-lg border border-white/[0.1] light:border-gray-200 bg-white/[0.02] light:bg-gray-50 flex items-center justify-center overflow-hidden shrink-0">
                      <img v-if="generalSettings.logo_data_url" :src="generalSettings.logo_data_url" :alt="$t('settings.organizationLogo')" class="h-full w-full object-contain" />
                      <Building2 v-else class="h-6 w-6 text-white/20 light:text-gray-300" />
                    </div>
                    <div class="space-y-1.5">
                      ...
                      <div class="flex items-center gap-2">
                        <Button variant="outline" size="sm" class="bg-white/[0.04] border-white/[0.1] text-white/70 hover:bg-white/[0.08] hover:text-white light:bg-white light:border-gray-200 light:text-gray-700 light:hover:bg-gray-50" @click="logoInput?.click()" :disabled="isUploadingLogo">
                          ...
                          {{ generalSettings.logo_data_url ? $t('settings.changeLogo') : $t('settings.uploadLogo') }}
                        </Button>
                        <Button v-if="generalSettings.logo_data_url" variant="ghost" size="sm" class="text-white/50 hover:text-white light:text-gray-500" @click="removeLogo" :disabled="isRemovingLogo">
                          {{ $t('common.remove') }}
                        </Button>
                      </div>
            ...
            <!-- Knowledge Base Embeddings Card (Gated on canWriteKnowledgeBase) -->
            <div v-if="canWriteKnowledgeBase" class="mt-6 rounded-xl border border-white/[0.08] bg-white/[0.02] light:bg-white light:border-gray-200">
              <div class="p-6 pb-3">
                <h3 class="text-lg font-semibold text-white light:text-gray-900">{{ $t('settings.knowledgeBaseEmbeddings') }}</h3>
                <p class="text-sm text-white/40 light:text-gray-500">{{ $t('settings.knowledgeBaseEmbeddingsDesc') }}</p>
              </div>
              <div class="p-6 pt-3 space-y-4">
                <div class="space-y-2">
                  <Label for="openai_embeddings_key" class="text-white/70 light:text-gray-700">{{ $t('settings.openaiEmbeddingsKey') }}</Label>
                  <Input
                    id="openai_embeddings_key"
                    type="password"
                    v-model="generalSettings.openai_embeddings_key"
                    :placeholder="generalSettings.has_openai_embeddings_key ? '••••••••••••' : 'sk-...'"
                  />
                  <p class="text-xs text-white/40 light:text-gray-500">{{ $t('settings.openaiEmbeddingsKeyHint') }}</p>
                </div>
                <div class="flex justify-end">
                  <Button variant="outline" size="sm" class="bg-white/[0.04] border-white/[0.1] text-white/70 hover:bg-white/[0.08] hover:text-white light:bg-white light:border-gray-200 light:text-gray-700 light:hover:bg-gray-50" @click="saveGeneralSettings" :disabled="isSubmitting">
                    <Loader2 v-if="isSubmitting" class="mr-2 h-4 w-4 animate-spin" />
